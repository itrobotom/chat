...
export { setName, getName, myNameServer }

let myNameServer; //глобальная переменная, чтобы в нее записать имя пользователя при запуске приложения (запуск)
...
async function getName() { //получим имя, а если нет доступа, то запросим окно ввода токена (если нет токена, сами переключимся на вкладку с вводом почты)
    ...
    try {
        ...
        if (response.ok) {
            ...
            console.log(`Запрошенное имя с сервера: ${myNameServer}`);
